...
def make_train_dataset(args, tokenizer, accelerator):
    ...
    if args.dataset_name is not None:
        ...
        dataset = load_dataset('json', data_files=args.train_data_json)
    ...
    def preprocess_train(examples):
        ...
        images = [Image.open(os.path.join(args.train_img_dir, image)).convert("RGB") for image in examples[image_column]]
        images = [image_transforms(image) for image in images]

        # conditioning_images = [image.convert("RGB") for image in examples[conditioning_image_column]]
        conditioning_images = [Image.open(os.path.join(args.train_img_dir, image)).convert("RGB") for image in examples[conditioning_image_column]]
