...
def add_lora_bin_old(lora_path, pipe, lora_weight=1.0):
...
def add_lora_bin(lora_path, pipe, lora_weight=1.0, only_unet=True):
    ...
    for lora_path, lora_weight in zip(lora_path_list, lora_weight_list):
        ...
        for k_lora, v_lora in tensors.items():
            ...
            if only_unet:
                ...
                cur_attr = pipe.unet if attr_name_list[0] == 'unet' else pipe.text_encoder
                start_idx = 1

            latest_attr_name = ''
            for idx in range(start_idx, len(attr_name_list)):
                attr_name = attr_name_list[idx]
                if is_int(attr_name):
                    cur_attr = cur_attr[int(attr_name)]
                    latest_attr_name = ''
...
def add_lora_unet_and_text_encoder_old(lora_file, pipe, lora_config_file, lora_weight=1.):
