...
class StableDiffusionCommonPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        controlnet_list = [ControlNetModel.from_pretrained(ctn) for ctn in controlnet_list]
        controlnet_list = torch.nn.ModuleList(controlnet_list)
    ...
    def text2img_prepare_latents(self, image, timestep, batch_size, num_images_per_prompt, height, width,dtype, device, generator=None, latents=None):
        if image is None:
            shape = (
                batch_size * num_images_per_prompt,
...
if __name__ == '__main__':
    ...
    prompt = 'a photo of a red car, anime style'
    noise = randn_tensor([1, 4, 512//8, 512//8], generator=generator, device=pipe.vae.device, dtype=pipe.vae.dtype)
    ...
    image.save('/share/wangqixun/workspace/tmp/0002.jpg')
    # text2img
    # image = pipe.text2img(
    #     prompt=prompt,
    #     height=512,
